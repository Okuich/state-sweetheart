    ...
    const step = (now: number) => {
      ...
      for (let i = 0; i < s.N; i++) {
        ...
        ctx.fill();
          ...
          // only tested against neighbors in the same or 8 surrounding cells.
          // For typical N=400-2500 with pRad ~ 50px this is 5–20× faster than
          // the all-pairs loop and lets the sim scale past 5k particles.
          if (pStr !== 0 && pRad > 0) {
            const cell = pRad;
            const gw = Math.max(1, Math.ceil(w / cell));
            const gh = Math.max(1, Math.ceil(h / cell));
            const nCells = gw * gh;
            // Counting-sort layout: cellStart[c] / cellEnd[c] index into `order`
            const cellCount = new Int32Array(nCells);
            const cellOf = new Int32Array(s.N);
            for (let i = 0; i < s.N; i++) {
              const cx = Math.min(gw - 1, Math.max(0, (s.x[i * 2] / cell) | 0));
              const cy = Math.min(gh - 1, Math.max(0, (s.x[i * 2 + 1] / cell) | 0));
              const c = cy * gw + cx;
              cellOf[i] = c;
              cellCount[c]++;
            }
            ...
            for (let cy = 0; cy < gh; cy++) {
              for (let cx = 0; cx < gw; cx++) {
                ...
                // Only visit each unordered pair of cells once: self + 4 of 8 neighbors
                for (let dyc = 0; dyc <= 1; dyc++) {
                  for (let dxc = -1; dxc <= 1; dxc++) {
                    ...
                    const cn = ny * gw + nx;
                    const bStart = cellStart[cn], bEnd = cellStart[cn + 1];
                    if (bStart === bEnd) continue;
                    const same = c === cn;
                    for (let ai = aStart; ai < aEnd; ai++) {
                      const i = order[ai];
                      const xi = s.x[i * 2], yi = s.x[i * 2 + 1];
                      const startB = same ? ai + 1 : bStart;
                      for (let bi = startB; bi < bEnd; bi++) {
                        const j = order[bi];
                        const dx = xi - s.x[j * 2];
