 ...
 * useful for breakdown screening, EMI shielding, sensor placement, and
 * PCB layout.
 *
...
export interface ElectrostaticSolution {
  ...
  E: Float64Array;
  /** Per-tet electric field E (flat xyz, length 3·nTets). */
  EPerTet: Float64Array;
...
export function solveElectrostatic(problem: ElectrostaticProblem): ElectrostaticSolution {
  ...
  const EPerTet = new Float64Array(nTets * 3);
  ...
  for (let t = 0; t < nTets; t++) {
    ...
    EPerTet[t * 3] = ex; EPerTet[t * 3 + 1] = ey; EPerTet[t * 3 + 2] = ez;
  ...
  for (let t = 0; t < nTets; t++) {
    for (let k = 0; k < 4; k++) {
      const vid = tets[t * 4 + k];
      E[vid * 3]     += EPerTet[t * 3];
      E[vid * 3 + 1] += EPerTet[t * 3 + 1];
      E[vid * 3 + 2] += EPerTet[t * 3 + 2];
      wAcc[vid] += 1;
    }
  }
  ...
  return { V, E, EPerTet, DPerTet, fieldMagnitudeTet, fieldMagnitude, intensity, solve };
}

/**
 * RK4 streamline tracer for E. Callers supply a `sampleE(x,y,z)` closure
 * (typically wrapping a kd-tree / octree spatial lookup of `E`) so the
 * tracer stays decoupled from any specific mesh accelerator.
 */
export interface FieldLineOptions {
  stepSize: number;
  maxSteps: number;
  /** Stop when |E| drops below this magnitude. */
  minMagnitude?: number;
  /** +1 follows E (high→low potential); −1 reverses. */
  direction?: 1 | -1;
}
...
export function traceFieldLine(
  ...
  const h = opts.stepSize * dir;
  let x = seed[0], y = seed[1], z = seed[2];
  for (let s = 0; s < opts.maxSteps; s++) {
    const k1 = sampleE(x, y, z);                            if (!k1) break;
    const k2 = sampleE(x + 0.5 * h * k1[0], y + 0.5 * h * k1[1], z + 0.5 * h * k1[2]); if (!k2) break;
    const k3 = sampleE(x + 0.5 * h * k2[0], y + 0.5 * h * k2[1], z + 0.5 * h * k2[2]); if (!k3) break;
    const k4 = sampleE(x + h * k3[0], y + h * k3[1], z + h * k3[2]);                   if (!k4) break;
    const dx = (h / 6) * (k1[0] + 2 * k2[0] + 2 * k3[0] + k4[0]);
    const dy = (h / 6) * (k1[1] + 2 * k2[1] + 2 * k3[1] + k4[1]);
    const dz = (h / 6) * (k1[2] + 2 * k2[2] + 2 * k3[2] + k4[2]);
    x += dx; y += dy; z += dz;
    pts.push(x, y, z);
    const m = Math.hypot(k1[0], k1[1], k1[2]);
    if (m < minMag) break;
  }
